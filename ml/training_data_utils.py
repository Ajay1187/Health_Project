"""Utilities for preparing disease training data from ml/dataset.csv."""
...
def prepare_dataframe_from_dataset(df: pd.DataFrame) -> pd.DataFrame:
    """Convert dataset.csv disease/symptom columns into a multi-hot feature table."""
    if "Disease" not in df.columns:
        raise ValueError("Expected dataset.csv schema with a 'Disease' column.")

    symptom_columns: List[str] = [column for column in df.columns if column.startswith("Symptom_")]
    ...
    for _, row in df.iterrows():
        ...
        for column in symptom_columns:
            symptom = normalize_symptom(str(row.get(column, "")))
    ...
    prepared_df = pd.DataFrame(encoded_rows)
