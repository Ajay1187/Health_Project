...
public class DiseaseDataRepository {
    ...
    public DiseaseResponse predictDisease(List<String> symptoms, int durationDays, float temperatureF) {
        ...
        for (DiseaseProfile profile : cachedProfiles.values()) {
            ...
            float coverage = (float) matchedCount / Math.max(1, normalizedSymptoms.size());
            float diseaseCoverage = (float) matchedCount / Math.max(1, profile.symptoms.size());
            float score = (coverage * 0.60f) + (diseaseCoverage * 0.25f)
                    + ((matchedSeveritySum / (float) Math.max(1, matchedCount * 7)) * 0.15f);

            if (score > bestScore) {
                bestScore = score;
                bestProfile = profile;
                bestMatchedCount = matchedCount;
                bestMatchedSeveritySum = matchedSeveritySum;
            }
        }
        ...
        float confidence = Math.min(0.98f, Math.max(0.45f, bestScore));
        int averageMatchedSeverity = bestMatchedCount == 0 ? inputSeverityScore : Math.round(bestMatchedSeveritySum / (float) bestMatchedCount);
        int severityScore = Math.max(1, Math.min(7,
                Math.round((averageMatchedSeverity * 0.6f) + (inputSeverityScore * 0.4f))));

        response.setPredictedDisease(bestProfile.diseaseName);
        response.setDescription(bestProfile.description);
        response.setPrecautions(bestProfile.precautions);
        response.setDoctorDetails(bestProfile.doctorDetails);
        response.setDietPlan(bestProfile.dietPlan);
        response.setExercise(bestProfile.exercise);
